    ...
    glob(src + glob_wildcard, (err, res) => {
        if (err) {
            ...
            fs.writeFile(fs_path.join(index_db_path, index_export_path), index.export(), { flag: "w" }, (err) => {
                ...
                console.log("File Path Indexing Complete")
...
function initializeSearch(query_string, callback) {
    if (!fs.existsSync(fs_path.join(index_db_path, preffered_indexes.exports_to))) {
        index_paths(preffered_indexes.path_url, preffered_indexes.glob_wildcard, preffered_indexes.exports_to, callback)
    } else {
        fs.readFile(fs_path.join(index_db_path, preffered_indexes.exports_to), (err, file_buffer) => {
            if (err) {
                callback(err);
            } else {
